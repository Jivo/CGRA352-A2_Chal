...
Mat calcDiffernece(const Mat&src1, const Mat&src2) {
	Mat diff = Mat::zeros(src1.rows, src1.cols, CV_32S);
	Mat mat1(src1.rows,src1.cols, CV_8U), mat2(src2.rows, src2.cols, CV_8U);
...
void drawSeam(Mat&src, const vector<int>& seam) {
	...
	for (int row = src.rows - 1; row >= 0; row--) {
		...
		for (int col = 0; col < src.cols; col++) {
			if (col >= seam.at(row) - (width / 2) &&
				col <= seam.at(row) + (width / 2)) {
